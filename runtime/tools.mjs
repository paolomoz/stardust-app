...
export const TOOL_SPECS = [
  ...
  { type: "function", function: { name: "write_file", description: "Write a UTF-8 file (creates parent dirs, overwrites). For a LARGE file (e.g. a full HTML page), write the first part here and add the rest with append_file in further calls — a single huge write can be truncated.", parameters: { type: "object", properties: { path: { type: "string" }, content: { type: "string" } }, required: ["path", "content"] } } },
  { type: "function", function: { name: "append_file", description: "Append UTF-8 content to a file (creates it if missing). Use to build a large file in chunks after an initial write_file.", parameters: { type: "object", properties: { path: { type: "string" }, content: { type: "string" } }, required: ["path", "content"] } } },
  ...
  { type: "function", function: { name: "emit_milestone", description: "Push a progress milestone to the live web UI. Call the INSTANT a phase boundary happens. Uplift shapes: extract.started; extract.seed{seed}; extract.tensions{items:[{n,text}]}; extract.brand_ready{brandReview,palette:[hex,…]}; direct.variants_ready{sharedFixes,variants:[{id,title,pitch,whatif,role,file,thumb}]}; prototype.variant_done{variant}; done. Iterate: iterate.done{variant,file} or iterate.answer. New direction (variant mode): variant.added{card:{id,title,pitch,whatif,role,file,thumb}} or variant.answer. Prototype phase (template mode): template.page_started{slug,title}; template.page_done{slug,title,file,thumb}; template.page_failed{slug,message}; template.answer. Deploy (EDS conversion mode): deploy.page_converted{slug}; deploy.bundle_ready{pages:[slug,…]}.", parameters: { type: "object", properties: { phase: { type: "string" }, event: { type: "string" }, data: { type: "object", description: "the milestone payload fields for this phase/event" } }, required: ["phase"] } } },
...
export function makeTools({ workdir, outputsDir, ingest }) {
  return {
    ...
    async append_file({ path, content }) {
      try { await mkdir(dirname(path), { recursive: true }); await appendFile(path, content ?? ""); return `appended ${(content ?? "").length}B to ${path}`; } catch (e) { return `[error] ${e.message}`; }
    },
...
export const LOCAL_TOOLS = new Set(["run_bash", "read_file", "write_file", "append_file", "edit_file"]);
